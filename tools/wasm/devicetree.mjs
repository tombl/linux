...
export function devicetree(tree, reservations = [], bootCpuId = 0) {
	...
	function bytes(buf) {
		...
		if (!(buf instanceof ArrayBuffer)) {
			throw new TypeError(`Expected ArrayBuffer, got ${buf[Symbol.toStringTag]}`);
		}
		...
		for (const [name, prop] of properties) {
			...
			switch (typeof prop) {
				...
				case "string":
					...
					break;
				case "object":
					value = Array.isArray(prop)
						? new Uint32Array(prop.map(byteswap32)).buffer
						: prop;
					break;
				case "undefined":
