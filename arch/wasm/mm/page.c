...
void arch_alloc_page(struct page *page, int order) {
	unsigned long desired = page_to_pfn(page), current = __builtin_wasm_memory_size(0);
	
	if (current < desired) {
		if (__builtin_wasm_memory_grow(0, desired - current) < 0) pr_err("failed to grow memory\n");
	}
}
