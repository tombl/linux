...
static struct wasm_sem *wasm_sem_from_fd(int fd, struct fd *f)
{
	*f = fdget(fd);
	...
	if (fd_file(*f)->f_op != &wasm_sem_fops) {
		fdput(*f);
...
SYSCALL_DEFINE5(wasm_sem_open, const char __user *, user_name, int, flags,
		umode_t, mode, unsigned int, value, u64 __user *, user_id)
{
	char name[NAME_MAX + 1];
	struct wasm_sem *sem;
	bool created = false;
	...
	if (put_user(sem->id, user_id)) {
		error = -EFAULT;
		goto out_remove_created;
	}
