...
char _etext[0], _einittext[0], _end[0];

#define INITCALL_LEVEL(n)						\
	initcall_entry_t *__initcall##n##_start, *__initcall##n##_end;
...
initcall_entry_t *__con_initcall_start, *__con_initcall_end;
const struct kernel_param *__start___param, *__stop___param;
const struct module_version_attribute *__start___modver, *__stop___modver;
const void *__start_notes, *__stop_notes;
void *__start_builtin_fw, *__end_builtin_fw;
...
#define SECTION_STRUCT(name, section) \
	SECTION(name, section.start, section.size, section.end)

	/* Keep in sync with %keep in arch/wasm/scripts/sections.pl. */

#define INITCALL_LEVEL(n) \
	SECTION(".initcall" #n ".init", __initcall##n##_start, _, \
		__initcall##n##_end)
...
#undef INITCALL_LEVEL
	...
	SECTION("__modver",		__start___modver,	_,	__stop___modver);
	SECTION(".note.Linux",		__start_notes,		_,	__stop_notes);
	SECTION(".builtin_fw",		__start_builtin_fw,	_,	__end_builtin_fw);
